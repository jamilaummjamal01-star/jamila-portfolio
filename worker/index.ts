...
const importFieldAliases: Record<keyof ImportKnowledgeItem, string[]> = {
  itemType: ["itemType", "item_type", "type", "тип", "тип записи"],
  speaker: ["speaker", "спикер", "сторона", "автор"],
  category: ["category", "категория", "раздел"],
  title: ["title", "название", "заголовок"],
  promptText: ["promptText", "prompt_text", "prompt", "вопрос", "формулировка"],
  shortText: ["shortText", "short_text", "короткий ответ", "краткий ответ"],
  fullText: ["fullText", "full_text", "подробный ответ", "полный ответ", "текст"],
  softText: ["softText", "soft_text", "мягкий вариант"],
  firmText: ["firmText", "firm_text", "твёрдый вариант", "твердый вариант"],
  clarificationText: ["clarificationText", "clarification_text", "уточняющий вопрос"],
  nextActionText: ["nextActionText", "next_action_text", "следующий шаг", "следующее действие"],
  avoidText: ["avoidText", "avoid_text", "чего не писать", "избегать"],
  diagnosticValue: ["diagnosticValue", "diagnostic_value", "значение для диагностики"],
  redFlagText: ["redFlagText", "red_flag_text", "красный флаг", "объяснение риска"],
  channel: ["channel", "канал"],
  tone: ["tone", "тон"],
  requiredLevel: ["requiredLevel", "required_level", "обязательность", "приоритет"],
  riskLevel: ["riskLevel", "risk_level", "риск", "уровень риска"],
  status: ["status", "статус"],
  sourceUrl: ["sourceUrl", "source_url", "ссылка", "ссылка на источник"],
  nicheSlugs: ["nicheSlugs", "niche_slugs", "niches", "niche", "ниши", "ниша"],
  stageSlugs: ["stageSlugs", "stage_slugs", "stages", "stage", "этапы", "этап"],
};

const importItemTypeAliases = new Map<string, string>([
  ["вопрос клиенту", "question_to_client"],
  ["вопрос клиента", "question_from_client"],
  ["ответ", "answer"],
  ["возражение", "objection"],
  ["ответ на возражение", "objection_response"],
  ["уточняющий вопрос", "clarifying_question"],
  ["первое сообщение", "first_message"],
  ["повторное сообщение", "follow_up"],
  ["подсказка диагностики", "diagnostic_hint"],
  ["проверка аудита", "audit_check"],
  ["красный флаг", "red_flag"],
  ["этическое правило", "ethical_rule"],
  ["блок кп", "proposal_block"],
  ["пакет", "package"],
  ["следующий шаг", "next_action"],
  ["основание для отказа", "refusal_reason"],
]);

function normalizeImportKey(value: string): string {
  return value.toLowerCase().replace(/ё/g, "е").replace(/[^a-zа-я0-9]+/giu, "");
}
...
function parseImportContent(content: string, format: string): Record<string, unknown>[] | Response {
  if (!content.trim()) return constructorError(422, "IMPORT_EMPTY", "Выберите непустой файл JSON или CSV.");
  if (new TextEncoder().encode(content).byteLength > maximumImportBytes) {
    return constructorError(413, "IMPORT_TOO_LARGE", "Файл импорта должен быть не больше 2 МБ.");
  }
  ...
  if (detectedFormat === "csv") {
    ...
    try {
      ...
      if (!Array.isArray(candidates)) {
        return constructorError(422, "IMPORT_JSON_SHAPE", "JSON должен содержать массив записей или поле items с массивом.");
      }
      rows = candidates.map(asRecord).filter((value): value is Record<string, unknown> => Boolean(value));
    } catch {
      return constructorError(422, "IMPORT_JSON_INVALID", "Не удалось прочитать JSON. Проверьте формат файла.");
    }
  }

  if (rows.length === 0) return constructorError(422, "IMPORT_NO_ROWS", "В файле не найдено строк для импорта.");
  if (rows.length > maximumImportRows) {
    return constructorError(413, "IMPORT_TOO_MANY_ROWS", `За один раз можно проверить не больше ${maximumImportRows} строк.`);
  }
  return rows;
}
...
function resolveImportReferences(value: unknown, references: Map<string, string>, label: string, errors: string[]): string[] {
  ...
  for (const candidate of splitImportList(value)) {
    ...
    if (!slug) errors.push(`${label} «${candidate}» не найдена`);
...
function normalizeImportedItem(
  ...
  const itemType = knowledgeItemTypes.has(rawItemType) ? rawItemType : importItemTypeAliases.get(rawItemType.replace(/ё/g, "е")) ?? rawItemType;
  const rawStatus = importString(readAliasedImportValue(record, "status"), 40).toLowerCase();
  const mappedStatus = rawStatus === "review" || rawStatus === "на проверке" || rawStatus === "approved" || rawStatus === "утверждено" ? "review" : "draft";
  if (rawStatus === "approved" || rawStatus === "утверждено") warnings.push("Статус изменён на «На проверке»: импорт не утверждает записи автоматически");

  const speakerValue = importString(readAliasedImportValue(record, "speaker"), 40).toLowerCase();
  const speakerAliases: Record<string, string> = { автор: "creator", креатор: "creator", клиент: "client", система: "system" };
  ...
  const requiredAliases: Record<string, string> = { обязательно: "required", рекомендуется: "recommended", опционально: "optional", условно: "conditional" };
  ...
  const riskAliases: Record<string, string> = { обычный: "normal", повышенный: "elevated", высокий: "high", отказ: "refusal" };
  ...
  const nicheSlugs = resolveImportReferences(readAliasedImportValue(record, "nicheSlugs"), niches, "Ниша", errors);
  const stageSlugs = resolveImportReferences(readAliasedImportValue(record, "stageSlugs"), stages, "Этап", errors);

  if (!knowledgeItemTypes.has(itemType)) errors.push("Не указан или не распознан тип записи");
  if (!category) errors.push("Не указана категория");
  if (!title) errors.push("Не указано название");
  if (!knowledgeSpeakers.has(speaker)) errors.push("Не распознан спикер");
  if (!knowledgeRequiredLevels.has(requiredLevel)) errors.push("Не распознана обязательность");
  if (!knowledgeRiskLevels.has(riskLevel)) errors.push("Не распознан уровень риска");
  if (sourceUrl && !isHttpUrl(sourceUrl)) errors.push("Ссылка на источник должна начинаться с http:// или https://");
  if (nicheSlugs.length === 0) warnings.push("Не указана ниша: запись будет универсальной");
  ...
  if (!item.promptText && !item.shortText && !item.fullText) warnings.push("У записи нет вопроса или основного текста");
  if ((riskLevel === "high" || riskLevel === "refusal") && !item.redFlagText) warnings.push("Для высокого риска желательно добавить объяснение");
...
async function previewImportRows(rows: Record<string, unknown>[], db: D1Database): Promise<ImportPreviewRow[]> {
  ...
  return rows.map((record, index) => {
    ...
    return {
      ...
      warnings: duplicate ? [...normalized.warnings, "Такая запись уже есть в базе или повторяется в файле"] : normalized.warnings,
...
async function readImportBody(request: Request): Promise<Record<string, unknown> | Response> {
  ...
  if (contentLength > maximumImportBytes + 300_000) return constructorError(413, "IMPORT_TOO_LARGE", "Файл импорта должен быть не больше 2 МБ.");
  try {
    const body = asRecord(await request.json());
    return body ?? constructorError(400, "INVALID_JSON", "Не удалось прочитать параметры импорта.");
  } catch {
    return constructorError(400, "INVALID_JSON", "Не удалось прочитать параметры импорта.");
  }
}
...
async function handleImportConfirm(request: Request, env: Env, identity: ConstructorIdentity): Promise<Response> {
  ...
  if (!importSourceKinds.has(sourceKind)) return constructorError(422, "IMPORT_SOURCE_INVALID", "Выберите корректный источник импорта.");
  if (!Array.isArray(body.items) || body.items.length === 0 || body.items.length > maximumImportRows) {
    return constructorError(422, "IMPORT_ITEMS_INVALID", `Подтвердите от 1 до ${maximumImportRows} проверенных записей.`);
  }
  const rawRows = body.items.map(asRecord).filter((value): value is Record<string, unknown> => Boolean(value));
  if (rawRows.length !== body.items.length) return constructorError(422, "IMPORT_ITEMS_INVALID", "В подтверждении есть некорректные записи.");
  ...
  if (readyRows.length === 0) return constructorError(409, "IMPORT_NOTHING_READY", "После повторной проверки не осталось новых записей для импорта.");
...
async function handleQualityDashboard(env: Env): Promise<Response> {
  ...
  const issueDefinitions = [
    ["missing_niche", "Без привязки к нише", "Записи могут быть универсальными, но их стоит проверить."],
    ["questions_without_answer", "Вопросы клиента без ответа", "Для вопроса нет короткого или подробного ответа."],
    ["answers_without_next_action", "Ответы без следующего шага", "После ответа клиенту не предложено конкретное действие."],
    ["duplicate_items", "Возможные дубли", "Одинаковые формулировки требуют ручной проверки."],
    ["stale_drafts", "Старые черновики", "Черновики не обновлялись больше 30 дней."],
    ["tariffs_due", "Тарифы требуют проверки", "Нет даты следующей проверки или она уже наступила."],
    ["high_risk_without_explanation", "Высокий риск без объяснения", "У высокого риска не заполнено объяснение красного флага."],
    ["stale_sources", "Устаревшая проверка источника", "Утверждённые записи не проверялись больше 180 дней."],
  ] as const;
